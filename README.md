...
- 🧠 **Arkham Intelligence Layer (V3)** — Cache-first wallet profiles, real trader win rates, settled market counts, smart-money PnL, and event liquidity heat
...
## What's New in v3.0.0 — Arkham Deep Intelligence

Poly Master v3 helps agents answer a richer user question: **"Who is behind this Polymarket activity, and are they any good?"** It combines the existing market/trading workflow with Arkham-derived wallet and event intelligence.

### V3 Intelligence Highlights

| Capability | What changed |
|-----------|--------------|
| Trader leaderboard | `poly-master-traders` can use cached Arkham leaderboard data with real `winRate` and `settledMarkets` instead of placeholder trade counts. |
| Wallet intelligence | New `poly-intel` actions expose cache-first wallet profiles, PnL, biggest win, leaderboard, top holders, hotspots, and network stats. |
| Portfolio enrichment | `poly-positions` may include Arkham `winRate`, `totalPnl`, and `biggestWin` when cached; otherwise it returns the original portfolio plus enrichment metadata. |
| Trending enrichment | `poly-trending` may merge Arkham event heat such as `totalVolume` and `uniqueTraders` into Gamma market data when cached. |
...
| Compare market heat before entering | "Show trending markets and include Arkham liquidity heat if available." |
| Explain smart-money context | "Does this whale have Polymarket history or no activity?" |

### What the Agent Should Tell Users

- If data is available, summarize the intelligence in plain language: win rate, settled markets, total PnL, biggest win, and visible wallet quality signals.
...
- If `meta.queuedForEnrichment=true`, return the normal Polymarket/Gamma result and tell the user: "Deep wallet intelligence is being prepared in the background; ask again later for Arkham fields."
- Do not imply that missing Arkham fields mean the user is safe to trade. Treat missing intelligence as incomplete information.
...
| Arkham Enrichment | Cached wallet `winRate`, `totalPnl`, and `biggestWin` may be attached when available |
...
### 🧠 Arkham Intelligence (V3)

| Action | Description |
|--------|-------------|
| `wallet-profile` | Cache-first wallet PnL, win rate, biggest win, and settled market activity |
| `leaderboard` | Arkham-backed trader leaderboard when cache is warm |
| `top-holders` | Event-level holder intelligence; requires an `event_id` |
...
>
> *"Show Arkham hotspots for Polymarket."*
>
...
| **Tools** | 30+ MCP tools for market data, trading, copy-trading, portfolio, hedge strategy, Arkham intelligence |
...
| `poly-intel` | Cache-only Arkham intelligence: wallet profile, leaderboard, top holders, hotspots, network stats |
...
- 🧠 **Arkham 深度情报层（V3）** — 缓存优先的钱包画像、真实胜率、已结算市场数、Smart-Money PnL 和事件资金热度
...
## v3.0.0 新增：Arkham 深度情报集成

Poly Master v3 帮助 Agent 回答更接近真实交易决策的问题：**“这个钱包是谁？它在 Polymarket 上到底表现怎么样？这个市场背后的资金热度如何？”** 它把原有市场发现、交易、跟单流程，与 Arkham 钱包/事件情报结合起来。

### V3 情报能力

| 能力 | 更新说明 |
|------|----------|
| 交易员榜单 | `poly-master-traders` 可读取 Arkham 缓存榜单，返回真实 `winRate` 和 `settledMarkets`，不再使用伪造交易笔数字段。 |
...
| 热门市场富化 | `poly-trending` 可将 Arkham 的 `totalVolume`、`uniqueTraders` 等资金热度合并到 Gamma 市场数据。 |
...
| 入场前比较市场资金热度 | “展示热门市场，如果有 Arkham 资金热度也一起给我。” |
...
- `meta.queuedForEnrichment=true` 时，先返回原有 Polymarket / Gamma 结果，并告诉用户：“深度钱包情报正在后台补全，稍后再问可以看到 Arkham 字段。”
- 不要因为 Arkham 字段缺失就建议交易；缺失情报只能代表信息不完整。
...
| 交易历史 | 完整的交易记录含时间戳 |
| 挂单查询 | 等待成交的未完成订单 |
| Arkham 富化 | 缓存命中时附加钱包胜率、总 PnL、最大盈利市场 |
...
| `leaderboard` | Arkham 缓存中的交易员榜单 |
| `top-holders` | 事件级持仓地址情报，需要 `event_id` |
...
| **工具数量** | 30+ MCP 工具，覆盖市场数据、交易、跟单、持仓管理、对冲策略、Arkham 情报 |
