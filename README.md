...
| Wallet profile | New `poly-master-traders` views expose cache-first wallet profiles, PnL, biggest win, leaderboard, top holders, and hotspots. |
| Portfolio enrichment | `poly-portfolio` (view: positions) may include Antalpha wallet data such as `winRate`, `totalPnl`, and `biggestWin` when cached; otherwise it returns the original portfolio plus enrichment metadata. |
| Trending enrichment | `poly-markets` (view: trending) may merge Antalpha event activity signals such as `totalVolume` and `uniqueTraders` into Gamma market data when cached. |
...
|-----------|----------------|
| Evaluate a wallet before copy-trading | "Profile this Polymarket wallet before I follow it: 0xABC..." (uses `poly-master-traders` with `view: wallet-profile`) |
...
Run `poly-strategy` (action: scan) daily. Collect T1/T2 signals when available. Execute small positions ($1–$10) consistently. Compound low-risk returns without directional exposure.
...
| Trade History | On-chain history via `poly-portfolio` (view: history) |
| Order List | All orders with status filter via `poly-order-status` (omit `order_id`) |
...
Exposed via `poly-master-traders` using the `view` parameter:
...
| `rank` | Top traders ranked by performance |

**Example conversations:**
> *"Use poly-master-traders (view: wallet-profile) to profile this wallet: 0xABC..."*
>
...
| `poly-markets` | Market discovery (`view`: `trending` by 24h volume / `new` recently created / `info` full market details (needs `market_id`) / `search` by `query` + optional `lang`) |
| `poly-buy` | Buy outcome tokens (market/limit) |
| `poly-sell` | Sell outcome tokens |
| `poly-order-status` | Order status: pass `order_id` for a single order's signing + CLOB fill status; omit `order_id` for the order list (optional `status`/`limit`) |
| `poly-portfolio` | Wallet portfolio (`view`: `positions` current holdings / `history` on-chain trade history) |
| `poly-master-traders` | Top traders ranked by performance plus Antalpha wallet and event data (`view`: `rank` / `leaderboard` / `top-holders` / `wallet-profile` / `hotspots`) |
...
| `poly-strategy` | Hedge strategy (`action`: `scan` for signals with full per-signal detail inline / `metrics` strategy dashboard / `dry-run` toggle dry-run flag, live execution not yet exposed via MCP) |
...
| 钱包画像 | `poly-master-traders` 新增 view，支持钱包 PnL、最大盈利、leaderboard、top holders、hotspots 等缓存优先查询。 |
| 持仓富化 | `poly-portfolio`（view: positions）命中缓存时会附加 `winRate`、`totalPnl`、`biggestWin`；未命中时保留原始持仓并返回补全标记。 |
| 热门市场富化 | `poly-markets`（view: trending）可将 Antalpha 的 `totalVolume`、`uniqueTraders` 等资金热度合并到 Gamma 市场数据。 |
...
**📍 场景四：被动生成 Alpha**
每天运行 `poly-strategy`（action: scan），稳定收集 T1/T2 信号，每次使用 5–10 USDC 小仓位。无需方向判断，也能持续捕捉结构性收益。
...
通过 `poly-master-traders` 的 `view` 参数提供：
...
**示例对话：**
> *"用 poly-master-traders（view: wallet-profile）看一下 0xABC... 的钱包画像"*
>
