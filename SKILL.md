...
| 🔎 **市场全文搜索** | `poly-markets`(view=search) 多语言搜索长尾市场（事件/球队/候选人/slug） |
| 🧠 **Antalpha 钱包画像** | 查询钱包 PnL、胜率、最大盈利、已结算市场活动 |
| 👥 **真实交易员榜单** | `poly-master-traders` 返回真实 `winRate` 与 `settledMarkets` |
| 🔥 **事件资金热度** | `poly-markets`(view=trending) / `poly-master-traders`(view=hotspots/top-holders) 可展示 totalVolume、uniqueTraders、top holders |
...
|------|-----------|-------------|
| `poly-markets` | `agent_id`, `view`(`trending`\|`new`\|`info`\|`search`), `limit?`, `category?`, `hours?`, `market_id?`, `query?`, `lang?`, `min_liquidity?`, `include_closed?` | 市场发现与查询，按 `view` 路由：**trending**（默认）热门市场（24h 成交量，缓存命中附加资金热度）；**new** 最近创建市场（`hours?`）；**info** 单市场完整信息（需 `market_id`：价格/成交量/Token ID/结果）；**search** 自由文本搜索长尾市场（需 `query`，按名称/球队/国家/候选人/slug，支持 zh-CN/zh-TW/en/ja/ko 翻译后查询）。`trending/new` 未返回用户问到的具体事件时用 `view: "search"` |
...
| `poly-order-status` | `agent_id`, `order_id?`, `status?`, `limit?` | 查询订单：传 `order_id` = 单笔签名+CLOB 成交状态（`pending_signature`/`submitted`/`matched`/`failed`，并自动修复 signed 未更新的订单）；不传 `order_id` = 订单列表（`status?` 过滤、`limit?`） |

### Copy Trading
| Tool | Parameters | Description |
|------|-----------|-------------|
| `poly-master-traders` | `agent_id`, `view?`(`rank`\|`leaderboard`\|`top-holders`\|`wallet-profile`\|`hotspots`), `limit?`, `sort_by?`, `address?`, `event_id?` | **view=rank**（默认）顶级交易员榜（真实 `winRate`/`settledMarkets`/copyScore）；**leaderboard** 历史盈利榜；**top-holders**（需 `event_id`=conditionId）某事件大户分布；**wallet-profile**（需 `address`）单钱包 PnL/胜率；**hotspots** 全网资金热点 |
...
| `poly-order-status` | `agent_id`, `order_id?`, `status?`, `limit?` | 跟单/普通订单列表（不传 `order_id`，`status?` 过滤） |

### Portfolio
| Tool | Parameters | Description |
|------|-----------|-------------|
| `poly-portfolio` | `agent_id`, `proxy_wallet`, `view?`(`positions`\|`history`), `limit?` | **view=positions**（默认）当前持仓：成本/市值/未实现 PnL（缓存命中附加 Antalpha 钱包数据）；**view=history** 链上历史成交记录 |
| `poly-order-status` | `agent_id`, `order_id?`, `status?`, `limit?` | 查未成交/挂单：不传 `order_id`，用 `status` 过滤（如 `status: "submitted"`） |

---

## V3 Antalpha Data Tools

### 什么时候使用 `poly-master-traders` 的情报 view

> 交易员榜单与情报已合并进 `poly-master-traders`，由 `view` 路由（`view=rank` 为默认的可跟单榜单）。

| 用户意图 | 推荐 view | 用户示例 |
|----------|-----------|----------|
| 跟单前判断钱包质量 | `wallet-profile` | “这个钱包 0xABC... 值得跟单吗？” |
| 找表现最稳定的交易员 | `leaderboard`（或默认 `rank`） | “按胜率和已结算市场数看顶级交易员。” |
| 看某事件背后是谁在持仓 | `top-holders` | “这个 event_id 的 top holders 是谁？” |
| 看全局资金热点 | `hotspots` | “现在 Polymarket 哪些事件资金最热？” |

### `poly-master-traders` 情报 view 参数规则

| view | Required parameters | 说明 |
|------|---------------------|------|
| `rank`（默认） | none | 可跟单交易员榜（winRate/copyScore/verdict） |
| `wallet-profile` | `address` | 钱包画像：PnL、胜率、最大盈利、已结算市场 |
| `leaderboard` | none | 历史盈利榜单 |
| `top-holders` | `event_id` | 事件级 top holders |
| `hotspots` | none | 全局热点事件 |

### 数据可用性解释

`poly-master-traders`（情报 view）和被富化的工具都会返回 `meta`。Agent 必须按以下方式解释：
...
| Tool | Parameters | Description |
|------|-----------|-------------|
| `poly-strategy` | `agent_id`, `action`(`scan`\|`metrics`\|`dry-run`), `limit?`, `min_tier?`, `enabled?` | **action=scan**（默认）触发一轮市场扫描，返回按覆盖率排序的对冲信号列表；**每条信号已含完整详情**（双腿 position、targetPrice/coverPrice、totalCost、coverage、expectedProfit、availableSize、tier、reasoning），无需再单独查询。**action=metrics** 策略监控看板；**action=dry-run** 查询/告知演练 vs 实盘模式（env 控制） |
...
```
poly-markets (view=trending / new) → 发现市场
poly-markets (view=info) → 查看详情
...
```
步骤 1: 调用 poly-strategy({ agent_id, action: "scan", limit: 5 })
...
当 `poly-portfolio`(view=positions) 未部署时：
```
...
Built by [Antalpha AI](https://ai.antalpha.com) | MCP: `https://mcp-skills.ai.antalpha.com/mcp` | v3.1.0

---

## Changelog

- **v3.1.0** — 工具收敛对齐 MCP（读类合并为带 `view`/`action` 参数的工具，写工具不变）：`poly-trending`/`poly-new`/`poly-market-info`/`poly-master-search-market` → **`poly-markets`**（`view: trending|new|info|search`）；`poly-positions`/`poly-history` → **`poly-portfolio`**（`view: positions|history`）；`poly-master-strategy-scan`/`-metrics`/`-dry-run` → **`poly-strategy`**（`action: scan|metrics|dry-run`）；`poly-intel` 并入 **`poly-master-traders`**（`view: rank|leaderboard|top-holders|wallet-profile|hotspots`，network-stats 下线）；`poly-confirm`/`poly-master-orders` 并入 **`poly-order-status`**（传 `order_id`=单查 / 不传=列表）。`poly-buy`/`poly-sell`/`poly-master-follow`/`poly-master-risk`/`poly-master-status`/`poly-master-pnl` 不变。
