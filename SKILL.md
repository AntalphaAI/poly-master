...
Poly Master v2 在 v1 交易/跟单基础上，新增 **PolyClaw 策略层**：利用 LLM 逻辑推理能力，扫描市场间的逻辑蕴含关系，发现接近无风险的对冲套利机会。所有订单通过 Polymarket Builder Program 归因，为 Antalpha 获取每周 USDC 收益分成。
...
| 🏗️ **Builder 归因** | 所有 CLOB 订单携带 X-Builder-Key，归因至 Antalpha Builder 账户 |
...
PolyMaster V2 所有 CLOB 订单均通过 Polymarket Builder Program 归因至 Antalpha Builder 账户：

- **X-Builder-Key**：每笔订单必须携带（缺失则拒单）
- **Relayer**：用户享受免 Gas 链上操作（需 Safe/Proxy 钱包）
- **收益**：按每周交易量发放 USDC 奖励分成
